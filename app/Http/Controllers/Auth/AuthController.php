...
//dd($data);
    ...
    public function postLogin(Request $request)
    {
		$username = $request->get('username');
		$password = encrypt(($request->get('password')));
		//dd([$request->get('password'), $password]);
		...
		try {
            ...
            $url = 'http://'.getServiceBaseURL().'/api/v1/auth/authenticate';
			$data = 'username='.$username."&encPassword=".$password;
			$data = [];
			$data['username'] = $username;
			$data['password'] = $password;
			
		
			$authData = sendPostRequest($url, $data, 'application/json');
			...
			if(isset($authData->status) && $authData->status!=null && $authData->status == "SUCCESS") {

				$token = $authData->token;
				//$token = \Crypt::encrypt($token);
				$otp = $authData->user->otp;
